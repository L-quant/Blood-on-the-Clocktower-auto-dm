<template>
  ...
  >
    ...
    <Menu ref="menu"></Menu>
    <AutomationPanel ref="automationPanel" />
    <EditionModal />
    <FabledModal />
    <RolesModal />
    <ReferenceModal />
...
export default {
  ...
  mounted() {
    // 监听打开自动化面板的事件
    this.$root.$on('open-automation-panel', () => {
      if (this.$refs.automationPanel) {
        this.$refs.automationPanel.toggle();
      }
    });
  },
  beforeDestroy() {
    this.$root.$off('open-automation-panel');
  },
  methods: {
