<template>
  <div class="token" @click="setRole" :class="[role.id, { hidden: isHidden }]">
    <!-- Hidden role card back -->
    <div v-if="isHidden" class="role-back">
      <span class="icon role-back-icon"></span>
      <div class="role-back-text">?</div>
    </div>
    
    <!-- Normal role card front -->
    <template v-else>
      <span
        class="icon"
        v-if="role.id"
