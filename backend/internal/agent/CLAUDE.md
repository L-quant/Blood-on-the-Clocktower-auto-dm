...
- `llm.go` → 旧版 LLM 客户端 (已弃用，用 llm/client.go 替代)
- `memory.go` → 旧版记忆管理与 RAG (已弃用，用 memory/manager.go 替代)
- `model_router.go` → 旧版模型路由 (已弃用，用 llm/router.go 替代)
- `orchestrator.go` → 旧版编排器 (已弃用，用 core/orchestrator.go 替代)
- `subagents.go` → 子代理工厂函数和辅助实现
