...
export default {
  ...
  computed: {
    ...
    ...mapState("privacy", { privacyState: state => state }),
    ...mapGetters({ nightOrder: "players/nightOrder" }),
    ...mapGetters("privacy", ["isPlayerOnlyMode", "canSeeRole"]),
    index: function() {
    ...
    shouldShowRole: function() {
      // 判断是否应该显示角色
      // 1. 说书人模式下总是显示
      if (!this.isPlayerOnlyMode) {
        return true;
      }
      
      // 2. 无说书人模式下，只显示自己的角色
      // 3. 游戏结束时显示所有角色
      const isGameEnded = this.session.isGameEnded || false;
      return isGameEnded || this.isMyPlayer;
    },
    voteLocked: function() {
  ...
  methods: {
    ...
    claimSeat() {
      ...
      // 在无说书人模式下使用带令牌的座位认领
      if (this.isPlayerOnlyMode && !this.player.id) {
        this.$store.dispatch('privacy/claimSeat', {
          playerId: this.session.playerId,
          seatIndex: this.index
        }).then(result => {
          if (result.success) {
            console.log('Seat claimed successfully with token');
          } else {
            alert(`Failed to claim seat: ${result.error}`);
          }
        });
      } else {
        // 说书人模式下使用原有的座位认领
